...
- `tls.enable: false` — when enabled with the corresponding CPA certificate/key settings, MiniCPA uses `https://` for readiness, status, and management URLs
...
- `cpa update check` exits non-zero if anything is outdated **or** either check (binary / panel) errors.
- `--binary`, `--panel`, and `--all` are mutually exclusive.

## MiniCPA self-upgrade

- `cpa upgrade check` queries `https://registry.npmjs.org` for `@astralyn/minicpa`'s `latest` dist-tag and compares exact semantic versions. It exits 1 when an update is available or the registry check fails; a local version newer than `latest` is reported as ahead and is never downgraded.
- `cpa upgrade` installs only when the current executable is proven to be a writable, direct npm global installation. It validates npm's global root and prefix, rejects links/junctions, and verifies the installed package manifest afterwards.
- npx caches, `npm link`, source checkouts, project dependencies, pnpm/yarn/bun layouts, ambiguous Windows project prefixes, and read-only global installs are not rewritten automatically. Use `npm install -g @astralyn/minicpa@latest` manually after resolving the installation method or permissions.
- npm is invoked without a shell and with fixed argument boundaries, `--ignore-scripts`, the official registry, and an exact version such as `@astralyn/minicpa@0.1.3`. It does not uninstall first and never requests elevation.
- `--force` reinstalls `latest` only when the current version equals `latest`; it does not authorize a downgrade.
- Updating the MiniCPA package does not stop, restart, or alter the managed CPA process. A successful command affects subsequent `cpa` invocations.
...
| Symptom | What to try |
|---------|-------------|
| `cpa start` says HTTP/HTTPS not ready | `cpa logs --err`, check `host`, `port`, and `tls.enable` plus CPA certificate/key paths in `config.yaml`, then `cpa restart` |
...
cpa update check  # check managed CPA binary/panel
cpa upgrade check # check the MiniCPA npm package
cpa doctor        # layout + binary + HTTP/HTTPS + GitHub probe
